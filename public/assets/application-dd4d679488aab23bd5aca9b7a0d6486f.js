...
$(document).ready(function() {
	...
	$(document).on("change", '.sale-reward-select', function() {
		$.ajax({
			type : "POST",
			url : '/sales/rewards_redemption',
			data : {
				sale_reward : {
					use_rewards : this.checked,
					sale_id : $(document).find('.sale_id').html()
				}
			},
			dataType : "script"
		});
	...
	$(document).on("change", "#line_item_price", function() {
		$.ajax({
			...
			data : {
				override_price : {
					price : $(this).val(),
					line_item_id : $(this).parent().parent().find('.line_item_id').val(),
					sale_id : $(document).find('.sale_id').html()
				}
			},
			dataType : "script",
	...
	$("#create_custom_customer").validate({
		rules : {
			"custom_customer[first_name]" : {
				required : true
			},
			"custom_customer[last_name]" : {
				required : true
			},
			"custom_customer[email_address]" : {
				email : true
			}
		}
	});
...
// $("#storeDropDown").change(function() {
	// $.get("update_store_select?store_select_id=" + $("#storeDropDown").val(), function(data) {
		// $("#branch_selection").html(data);
	// });
// }); 
